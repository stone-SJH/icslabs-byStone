## pipe-full.hcl 5130379072 shijiahao ##
#/* $begin pipe-all-hcl */
####################################################################
...
## optimize:
## 1.add 'iaddl'&'ileave',finish their logic,just like "seq-full.hcl"
## after modifying ncopy.ys and this, i can get 8.6+CPE.
## 2.i consider a situation where a instruction is writing a register from 
## memory and then the next instruction gets the value and write it to the 
## memory. since it doesnt need any value got from Decode stage, we didnt 
## deal with this situation in "L/U" hazard, but optimize it can help reduce
## 1bubble&1stall, i call this "l/u 2".all this situation is "IMRMOVL\IPOPL\
## ILEAVE" in Memory&"IRMMOVL\IPUSHL" in Execute,which M_dstM==E_srcA!=E_srcB
## here in ncopy.ys we can only meet "IMRMOVL->IRMMOVL" situation, so we can 
## also only write this one.(i concluded all this situation.)
## 3.i found in the "PUSHL->RET" situation, the value stored in the %ebp 
## register is return address, so i can reduce 3bubble to 1bubble in this 
## situation.
## 4.i mark all the place i optimized using this structure "##XXXX##" so you can## find them easily.
## final CPE:7.71 
## 9/24/2014 
####################################################################
