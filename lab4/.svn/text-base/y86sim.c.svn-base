    ...
    switch (icode) {
      ...
      case I_MRMOVL: /* 5:0 regB:regA imm */
        {
              ...
              long_t res=0;
              long_t valB=get_reg_val(sim->r,regB);
              if (!get_long_val(sim->m, valB+offset, &res)) {
                   err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc,valB+offset);
                   return STAT_ADR;}
      ...
      case I_JMP: /* 7:x imm */
     {
      ...
      case I_RET: /* 9:0 */
      {
              ...
              set_reg_val(sim->r,4,esp+4);
              long_t retaddr=0;
              if (!get_long_val(sim->m, esp, &retaddr)) {
                   err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc,esp);
                   return STAT_ADR;}
              sim->pc=retaddr;
              break;
