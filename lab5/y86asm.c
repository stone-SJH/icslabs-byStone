...
cont:
    ...
    switch (HIGH(inst->code)) {
      ...
      case I_POPL: {/* B:0 regA:F - e.g., popl %ebp */
        ...
        /* set y86bin codes */
        y86bin->codes[1]=HPACK(reg,REG_NONE);
        goto cont;
      }
      ...
      case I_IRMOVL: {  /* 3:0 Imm, regB - e.g., irmovl $-1, %ebx */
        ...
        y86bin->codes[1]=HPACK(REG_NONE,reg);
      ...
      case I_CALL: {/* 8:x dest - e.g., call Main */
        ...
        add_reloc(name,y86bin);
        goto cont;
      }/*no codes just symbol.so neednt change y86bin.codes*/
      ...
      case I_DIRECTIVE: {
        ...
        switch (LOW(inst->code)) {
          case D_DATA: {    /* .long data - e.g., .long 0xC0 */
            ...
            if (ret==PARSE_SYMBOL){
                 add_reloc(name,y86bin);
                 goto cont;/*the last error...i must admit it's so annoying to find this.*/
            }
            long *long_one=(long *)y86bin->codes;
            *long_one=value;
            goto cont;
          }
...
int relocate(void)
{
    ...
    while (rtmp) {
        ...
        if (type==I_IRMOVL) new_addr=(long*) &rtmp->y86bin->codes[2];
        else if (type==I_CALL||type==I_JMP) new_addr=(long*)&rtmp->y86bin->codes[1];
...
int binfile(FILE *out)
{
    ...
    while(cur_line){
       ...
       while (dup_line){
           if(dup_line->y86bin.bytes!=0){
               check=1;
               break;
           }
           else dup_line=dup_line->next;
       }
       if (cur_pos<cur_line->y86bin.addr && check){
           int differ=cur_line->y86bin.addr-cur_pos;
           int i=0;
           for (i=0;i<differ;i++)
               fprintf(out,"%c",0);
           cur_pos=cur_line->y86bin.addr;
       }
       fwrite(cur_line->y86bin.codes,sizeof(byte_t),cur_line->y86bin.bytes,out);
       cur_pos+=cur_line->y86bin.bytes;
